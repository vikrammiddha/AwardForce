                  ...
                  OpenFB.get('/me').success(function (user) {
                      ...
                      var result = push.registerPush(function (result) {
                        if (result.type === 'registration') {
                            var userInfoData = {name : user.name, email: user.email, imageurl:encodeURIComponent('https://graph.facebook.com/'+user.id+ '/picture?width=400&height=400'),token:result.Id};
                            userStore.setUserInfo(userInfoData,function(data){
